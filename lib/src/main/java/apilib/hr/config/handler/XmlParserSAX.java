package apilib.hr.config.handler;
...
import apilib.hr.config.object.ServletMappingObject;
import apilib.hr.config.object.ServletObject;

public class XmlParserSAX {
	private static final Logger log = LogManager.getLogger(XmlParserSAX.class);
			...
			for(int i = 0; i < servletObjectList.size(); i++) {
				for(int j = 0; j < servletMappingObject.size(); j++) {
					if (servletObjectList.get(i).getServletName().trim().equals(servletMappingObject.get(j).getServletName().trim())) {
						...
						int beginLocationEwalletName = functionName.indexOf("function.") + 9;
						int endLocationEwalletName = functionName.indexOf('.', beginLocationEwalletName);
						String eWalletName = functionName.substring(beginLocationEwalletName, endLocationEwalletName);
						...
						resultMap.put(eWalletName + dtoSimpleName, sbBaseUri.append(servletMappingObject.get(j).getUrlPattern().trim()).toString());
						break;
					}
				}
			}
